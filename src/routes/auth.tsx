    ...
    <div className="min-h-dvh bg-background flex items-center justify-center p-6">
      <div className="w-full max-w-md">
        <Link to="/" className="block text-center mb-10">
          <span className="font-serif text-3xl font-medium tracking-tight text-foreground">GapFriend</span>
          <span className="block text-xs font-mono uppercase tracking-widest text-terracotta mt-1">find your gap</span>
        </Link>
        ...
        <div className="bg-card rounded-3xl border border-border p-8 shadow-warm">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <Input id="password" type="password" required minLength={6} value={password} onChange={(e) => setPassword(e.target.value)} className="mt-1.5" autoComplete={mode === "signup" ? "new-password" : "current-password"} />
            </div>
          ...
          <p className="mt-6 text-center text-sm text-muted-foreground">
            ...
            <button onClick={() => setMode(mode === "signin" ? "signup" : "signin")} className="text-terracotta font-medium hover:underline">
