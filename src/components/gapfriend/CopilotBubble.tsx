...
function quickPromptsFor(path: string): { label: string; prompt: string }[] {
  if (path.endsWith("/gaps")) {
    return [
      { label: "Suggest 5 gaps", prompt: "Suggest 5 specific market gaps for me based on my profile. Use add_gap_cards." },
      { label: "Why these?", prompt: "Explain why the gaps you suggested fit my skills and constraints." },
    ];
  }
  if (path.endsWith("/brief")) {
    return [
      { label: "Draft brief", prompt: "Turn the selected gap into a tight opportunity brief (persona, problem, angle, business_model). Use save_opportunity_brief." },
      { label: "Sharpen angle", prompt: "The angle in my brief is too generic. Sharpen it." },
    ];
  }
  if (path.endsWith("/identity")) {
    return [
      { label: "Name it", prompt: "Suggest 5 names, 5 domain options, a tagline, and a positioning sentence. Use save_identity." },
      { label: "Refine identity", prompt: "Refine my identity (name + tagline + positioning) to feel more honest and specific." },
    ];
  }
  if (path.endsWith("/board") || path.endsWith("/roadmap")) {
    return [
      { label: "Plan my week", prompt: "Add 3 concrete tasks for THIS WEEK based on everything we know. Use add_tasks with column_name=this_week." },
      { label: "What's next?", prompt: "What's the single next thing I should do today?" },
    ];
  }
  if (path.endsWith("/money")) {
    return [
      { label: "Suggest math", prompt: "Propose realistic money settings — currency, monthly income_target, price_per_unit, hours_per_week, and 3 scenarios. Use save_money." },
    ];
  }
  if (path.endsWith("/channels")) {
    return [
      { label: "Suggest channels", prompt: "Recommend 3 channels for me to be visible on, with rationale, pros, cons, guide. Use save_channels." },
    ];
  }
  return [
    { label: "What now?", prompt: "Look at my project and tell me the single most useful next step. Don't ask questions — pick one and explain." },
    { label: "Find a gap", prompt: "Suggest 5 specific market gaps for me based on my profile. Use add_gap_cards." },
  ];
}
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4">
        {!hasMessages && (
          <div className="bg-muted/60 rounded-2xl rounded-tl-sm p-3.5 text-sm leading-relaxed">
            Hey — I'm GapFriend. I can see what you're looking at and act on it. Pick a quick action below or ask me anything.
          </div>
        )}
        {messages?.map((m) => (
          <div
